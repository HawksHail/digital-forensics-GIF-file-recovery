...
int init(FILE *pFile)
{
	...
	// printf("\nSuper Block 0\n");
	// sbPrintSuperblocks(&gPrimarySuperblock);
	...
	// 	return 0;
	// }
	tSuperblock sb = gPrimarySuperblock;
	int x = 0;
...
void sbGetSuperblock(FILE *pFile, int sbNum, tSuperblock *buff) {
	tSuperblock primarySB;
	INT2 i2RetVal1, i2RetVal2;
	// printf("BYTE NUMBER %i",pow(gBlockSize, 2) * 8 * sbNum);
	// pow(gBlockSize, 2) * 8 * i8BlockGroupNumber[i2Index1 - 1]
	i2RetVal1 = fseek(pFile, pow(gBlockSize, 2) * 8 * sbNum, SEEK_SET);
	if(i2RetVal1 != 0) {
		if (ferror(pFile)) {
			perror("fseek()");
			fprintf(stderr,"fseek() failed in file %s at line # %d\n", __FILE__,__LINE__-5);
	...
	if(i2RetVal2 != 1)
		if(ferror(pFile))
			printf("error reading filesystem\n");
	rewind(pFile);
	// printf("CHECK primary Magic Number: 0x%04X\n", primarySB.s_magic);
	*buff = primarySB;
	// printf("CHECK buff Magic Number: 0x%04X\n", buff->s_magic);
}
...
void sbPrintSuperblocks(tSuperblock *pSuperblock) {
	...
	// for(i2Index1 = 0; i2Index1 < SB_ARR_SZ; i2Index1++) {
		...
		printf("\n");
		printf("Free blocks: %d\n", pSuperblock->s_free_blocks_count);
		printf("Free inodes: %d\n", pSuperblock->s_free_inodes_count);
...
{
	return gPrimarySuperblock.s_inodes_per_group;
}
...
int getInodeSize()
{
	return gPrimarySuperblock.s_inode_size;
}
